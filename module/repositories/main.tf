...
resource "artifactory_virtual_generic_repository" "virtual_generic_repos" {
  for_each = { for r in artifactory_local_generic_repository.generic_repos : r.key => r.value }

  key                        = lower("${each.value.team}-${each.value.package_type}-${each.value.suffix == "dev-local" ? "dev" : "prod"}")
  repositories               = concat(
    [each.key],
    local.remote_repos.generic
  )
  default_deployment_repo    = lower(each.key)
  repo_layout_ref            = each.value.repo_layout_ref
  description                = each.value.description
  notes                      = each.value.notes
  depends_on = [
...
resource "artifactory_virtual_helm_repository" "virtual_helm_repos" {
  for_each = { for r in artifactory_local_helm_repository.helm_repos : r.key => r.value }
...
resource "artifactory_virtual_npm_repository" "virtual_npm_repos" {
  for_each = { for r in artifactory_local_npm_repository.npm_repos : r.key => r.value }
...
resource "artifactory_virtual_nuget_repository" "virtual_nuget_repos" {
  for_each = { for r in artifactory_local_nuget_repository.nuget_repos : r.key => r.value }
...
resource "artifactory_virtual_pypi_repository" "virtual_pypi_repos" {
  for_each = { for r in artifactory_local_pypi_repository.pypi_repos : r.key => r.value }
...
resource "artifactory_virtual_docker_repository" "virtual_docker_repos" {
  for_each = { for r in artifactory_local_docker_repository.docker_repos : r.key => r.value }

  key                        = lower("${each.value.team}-${each.value.package_type}-${each.value.suffix == "dev-local" ? "dev" : "prod"}")
  repositories               = concat(
    [each.key],
    local.remote_repos.docker
  )
  default_deployment_repo    = lower(each.key)
  repo_layout_ref            = each.value.repo_layout_ref
  description                = each.value.description
